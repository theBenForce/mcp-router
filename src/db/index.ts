...
function pushSchema(db: Database) {
  try {
    ...
    // Migration: add metadata columns to mcp_servers before any potential table recreate
    ...
    try {
      ...
      // CHECK constraint failed - recreate table while preserving ALL columns including metadata
      db.exec(`
        ...
        INSERT INTO mcp_servers_new (id, name, description, server_version, server_title, instructions, website_url, icons_json, transport_type, config_json, auth_type, auth_data_json, status, last_error, created_at, updated_at)
          SELECT id, name, description, server_version, server_title, instructions, website_url, icons_json, transport_type, config_json, auth_type, auth_data_json, status, last_error, created_at, updated_at FROM mcp_servers;
